...
impl DavInner {
    ...
    fn liveprop_remove(&self, prop: &Element, can_deadprop: bool) -> SC {
        match prop.namespace.as_ref().map(|x| x.as_str()) {
            ...
            Some(NS_APACHE_URI) | Some(NS_XS4ALL_URI) | Some(NS_MS_URI) => SC::FORBIDDEN,
            _ => SC::CONTINUE,
        }
    }
