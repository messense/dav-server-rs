...
impl Server {
    pub fn new() -> Self {
        Server{
            basedir: "/home/mikevs/devel/rust/webdav-server-rs/html".into(),
            icondir: "/usr/share/apache2/icons".into(),
            homedir: "/home".into(),
        }
    }

    fn authenticate(&self, req: &Request, res: &mut Response) -> Result<AuthInfo, StatusCode> {
        // we must have a login/pass
        // some nice destructuring going on here eh.
        let (user, pass) = match req.headers.get::<Authorization<Basic>>() {
            ...
            _ => {
                res.headers_mut().set(headers::WWWAuthenticate(
                            "Basic realm=\"webdisk\"".to_string()));
        ...
        Ok(AuthInfo {
            ...
            basedir: PathBuf::from("/home/mikevs/dir"),
            uid: 1000,
            gid: 1000,
        })
    }
...
impl Handler for Server {
    fn handle<'a, 'k>(&'a self, req: Request<'a, 'k>, mut res: Response<'a>) {
        ...
        let authinfo = match self.authenticate(&req, &mut res) {
            Ok(a) => a,
            Err(s) => { *res.status_mut() = s; return },
        };

        // get request path
        let webpath = match webpath::WebPath::from_uri(&req.uri) {
            Ok(p) => p,
            Err(_) => {
                *res.status_mut() = StatusCode::BadRequest; return
            },
        };

        // get filesystem basedir, url prefix, and path.
        let (base, prefix, path) = match webpath.split_user_path() {
            Some((u, b, p)) => {
                // check if name is same as username
                let name = String::from_utf8_lossy(u);
                if name != authinfo.username {
                    *res.status_mut() = StatusCode::Unauthorized;
                    return;
                }
                (authinfo.basedir, b.to_vec(), p)
            },
            // no user -- take server basedir.
            None => (self.basedir.clone(), b"".to_vec(), webpath.clone()),
        };

        // build davhandler

        let fs = localfs::LocalFs::new(base, authinfo.uid, authinfo.gid);
        let dav = DavHandler::new(&prefix, path, fs)
            .allow(Method::Get)
